...
whole series with the book's tie rule (Appendix F).

Pure functions, no I/O — trivially testable and reused by the result emitter.
Totals are DERIVED from per-sub-game results, never trusted from a claim.
"""
...
def score_subgame(result: str, roles: dict[str, str], scoring: dict) -> dict[str, int]:
    ...
    Any other outcome (timeout / tamper_forfeit / stopped) is 0/0.
    """
...
def aggregate(subgame_scores: list[dict[str, int]], tie_score: int) -> dict:
    ...
    Returns total_score per group, sub_games_won per group, tied-sub-game count,
    winner_group (None on a series tie), and series_tie. On a two-group series tie
    the book's rule grants each group ``tie_score``.
    """
