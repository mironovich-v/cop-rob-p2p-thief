...
Every LLM / email / network call goes through `execute()`: rate limiting (from
config), FIFO queuing on overflow, retry on transient provider errors, and call
logging. No external call may bypass it.
"""
...
class ApiGatekeeper:
    """Centralized external-call manager with rate limiting and queuing."""

    def __init__(self, config: ConfigManager, service: str, clock=None) -> None:
        ...
        self._limiter = RateLimiter(limits, config.rate_limits["queue"], clock=clock)
        self._calls_total = 0
