...
All limits come from config (`rate_limits.json`); nothing is hard-coded. An
injectable clock keeps tests deterministic and instant. When the per-minute
window is full a caller queues (up to `max_depth`) and polls until a slot frees
or it times out — overflow queues, it never silently drops.
"""
...
class RateLimiter:
    ...
    def __init__(self, limits: dict, queue_cfg: dict, clock=None) -> None:
