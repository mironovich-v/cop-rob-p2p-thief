...
| FR-8 | Orchestrator FSM; idempotency, monotonic ids, timeouts, retries, watchdog, restart | Book ch.8; Ref `peer/runtime*` | 2/6/7 | DONE (turn loop + handshake + watchdog timeout + mutual audit/tamper_forfeit 2.5b; strict step-monotonic dedup drops stale/dup/out-of-order 7.8; dead peer fails closed via timeout) | `test_runtime` (2-peer + dup-delivery), `test_turn_handler`, `test_series` |
