...
| 2026-08-24 | Analysis (docs) | docs-police-endgame-postmortem | (pending) | Fable 5 · high | pursuit-geometry reconstruction of g2/g4/g6 from sealed logs; police failure root-caused (dodge cycle + walls removed); PRD_strategy_brains §8 | ~0.3 session | in review |
| 2026-08-24 | Bug fix (export) | fix-export-ship-img | (pending) | Fable 5 · high | README embedded img/ screenshots the export never shipped (broken in both role repos); img/ now rides the git ls-files rule + markdown-image resolution test | ~0.2 session | in review |
| 2026-08-24 | Bug fix (export) | fix-export-ship-logs | (pending) | Fable 5 · high | tracked logs/ evidence (self-play series, friendlies, warm-up) + README replay-example target never shipped; logs joins the ls-files ship rule, only-tracked assertion added | ~0.2 session | in review |
| 2026-08-24 | Bug fix (publish) | fix-publish-force-add-logs | (pending) | Fable 5 · high | publish's add -A obeyed shipped .gitignore and dropped logs/ despite export fix #114; force-add logs like results, print both counts | ~0.1 session | in review |
